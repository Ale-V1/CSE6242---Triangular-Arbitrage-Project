...
def insert_opportunities_batch(conn, opportunities_df, run_id, run_time, run_parameters):
    """
    Bulk insert opportunities into database
    """
...
def get_all_trading_pairs(conn, table_name='ohlc_data'):
    ## replaced {table_name} with all_pairs
    """Get unique trading pairs from database"""
    query = f"""
    SELECT DISTINCT 
        base_currency,
        quote_currency
...
def get_unique_timestamps(conn, table_name, config):
    """Get all unique timestamps in the data range"""
    ...
    # hard_coded {table_name} to all_open_times. In live nevironment this table would be updated as the price data is loaded
    query = f"""
...
def fetch_price_data_for_timestamps(db_params, table_name, timestamps, config):
    """
...
# ============================================================================
# ARBITRAGE CALCULATION (Same logic, optimized for parallel)
# ============================================================================
        ...
        for triangle in triangles:
            ...
            for path in paths:
                ...
                if leg1 and leg2 and leg3:
                    ...
                    min_volume = min(leg1['volume'], leg2['volume'], leg3['volume'])
...
def process_timestamp_batch(args):
    ...
    """
    db_params, table_name, timestamp_batch, triangles, config = args
    
    try:
        # Fetch data for this batch of timestamps
        price_data = fetch_price_data_for_timestamps(
            db_params, table_name, timestamp_batch, config
        )
...
def detect_arbitrage_parallel(conn, config, table_name='ohlc_data', 
    ...
    if rebuild_triangles or not os.path.exists(config.triangle_cache_file):
        ...
        pairs_df = get_all_trading_pairs(conn, table_name)
    ...
    all_timestamps = get_unique_timestamps(conn, table_name, config)
    ...
    # Prepare database connection parameters for workers
    db_params = {
        'host': conn.info.host,
    ...
    worker_args = [
        (db_params, table_name, batch, triangles, config)
        ...
        for i, result_df in enumerate(tqdm(
            ...
            if len(result_df) > 0:
                inserted = insert_opportunities_batch(
                    conn, result_df, config.run_id, config.run_time, run_parameters
                )
...
def get_run_statistics(conn, run_id):
...
def get_top_opportunities(conn, run_id, limit=10):
...
def main():
    ...
    # Database
    parser.add_argument('--host', default='34.73.134.13')
    parser.add_argument('--database', default='postgres')
    parser.add_argument('--user', default='postgres')
    parser.add_argument('--password', default='dva_access')
    ...
    try:
        ...
        if args.analyze:
            ...
            stats = get_run_statistics(conn, args.analyze)
            print("\nRun Statistics:")
            print(stats.to_string(index=False))
            
            top = get_top_opportunities(conn, args.analyze, 10)
            print("\nTop 10 Opportunities:")
            print(top[['timestamp', 'path', 'profit_net_pct']].to_string(index=False))
        ...
        if args.compare:
            ...
            comparison = compare_runs(conn, args.compare)
        ...
        run_id, total_inserted = detect_arbitrage_parallel(
            conn, config, args.table, args.rebuild_triangles
        )
